...
**阶段**：Q1 P0-2 + P1-1 完整修复完成；**Q2 6 项修复正在执行中**（基于 `issue/q1234_fix_plan.md` 评审）
**时间锚点**：2026-09-12 12:50 UTC+8
**关键结果**：Q1 综合分 50.7 / 100（扣分前 64.6；扣分 -30 封顶）
**当前子任务**：Q2 修复（P0-1 注册率维度删除 / P0-2 潜力挖掘型启发式 / P1-1 cluster 3 注脚 / P1-2 映射表 / P2-1 多指标 / P2-2 雷达图）
...
## 0.2 Q2 6 项修复进度（与 `issue/q1234_fix_plan.md` Q2 节对齐）

| S | 阶段 | 状态 | 关键产物 |
|---|------|------|----------|
| S1 | 备份 + TodoWrite 建任务表 | ✅ 完成 | `results/snapshots/pre_q2_fix_20260912/` 含 5 文件 + q2_classify.py.bak + q2_method.md.bak |
| S2 | Q2.1 P0-1 修复：删除注册率维度（6→5 维） | ✅ 完成 | `src/q2_classify.py` 修改 + q2_method.md 同步 |
| S3 | Q2.2 P0-2 修复：重写 name_clusters 启发式 | ✅ 完成 | 8 类中 1 类命名为"潜力挖掘型"（122 词）|
| S4 | Q2.3 P1-1 修复：cluster 3 注脚 | ✅ 完成 | q2_cluster_summary.csv 备注列加 ⚠️ 注脚 |
| S5 | Q2.4 P1-2 修复：paper.md 5.2.3 映射表 | ✅ 完成 | paper.md 表 5-2 含 5×5 题面→业务类映射 |
| S6 | Q2.5 P2-1 修复：选 K 多指标交叉验证 | ✅ 完成 | q2_internal_metrics.csv (4 指标) + 子图 |
| S7 | Q2.6 P2-2 修复：5 类业务雷达图 | ✅ 完成 | q2_class_radar.png (5 类 4 维) |
| S8 | 验证：跑 q2_classify + check_q2.py 11/11 PASS | ✅ 完成 | 全部产物清单 + tools/check_q2.py |
| S9 | 文档同步：q2_method.md + paper.md + WORK_STATE + RUN_LOG | ✅ 完成 | q2_method.md 重写 + paper.md §5.2.1-5.2.4 |

---

## 0.3 Q2 修复结果（最终）

| 业务类 | 关键词数 | 占比 | 题面对应 | 业务动作 |
|---|---|---|---|---|
| 高价值转化型 | 145 | 6.5% | 黄金词 | 加预算、扩匹配 |
| 稳定拓展型（含一/二/三）| 634 | 28.5% | 重点词 | 维持、优化出价 |
| 潜力挖掘型 | 122 | 5.5% | 潜力词 | 优化创意、落地页 A/B |
| 高成本浪费型 | 1 | 0.05% | 问题词 | 暂停、个案处理 |
| 低效长尾型（含一/二）| 1,325 | 59.5% | 无效词 | 清理、降价 |
| **合计** | **2,227** | **100%** | — | — |

**验证**：`python tools/check_q2.py` → 11/11 PASS
