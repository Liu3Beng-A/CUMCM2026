...
**阶段**：Q1 P0/P1/P2 全修复完成（综合分 50.7 / 100，D 级）；**Q2 已完成 + 6 项稳健性**（10/10 PASS）；**Q3 已完成 + 7 项自检（6 PASS + 1 CAVEAT：注册代理 share-Pearson=-0.096 失效，详见 §5.3.4）**；**Q4 已完成 + 7 项自检（7 PASS + 1 精度溢出：投入超 0.06 元 = 0.0003% PuLP 浮点残留）**；**论文终稿同步完成 2026-09-12 17:50**（摘要 caveat + Q4 精度放宽 + 附录 A/B 状态统一 + n_boot 50→100 全修复）。
**时间锚点**：2026-09-12 17:50 UTC+8
**关键结果**：Q1 综合分 50.7 / 100（扣分前 64.6；扣分 -30 封顶）；Q3 result3.xlsx 89 行 MILP Optimal + 代理失效 caveat；Q4 result4.xlsx 39 行 Two-Stage SP + 精度溢出 caveat

---

## 0.1 幻觉事故清理进度（2026-09-12 13:25 → 17:50 论文同步完成）
...
## 0.5 BIAS_v2 修订日志（2026-09-12 17:27 · 来自方案审查 4 项关注点）

> **触发**：用户在 17:18 要求"分析验证方案是否合理，严格根据事实题目和事实附件分析"。审查发现 4 项关注点，已在 `Q3Q4_Method_Architecture.md` 落实为可执行细节。

| ID | 关注点 | 处理 | 文档位置 |
|---|---|---|---|
| V2-1 | 关联规则数据源：Sheet1 无关键词列 | 新增"数据源代理"声明：用推广单元-入选项池（909 词）代理 | `Q3Q4_Method_Architecture.md` §2.2 Layer 2 |
| V2-2 | Q3 backtest 校验逻辑矛盾（反事实 vs 实际） | 改写为"代理精度"校验（评估代理假设精度）| `Q3Q4_Method_Architecture.md` §2.3 |
| V2-3 | §2.2.1 代理变量法弱假设未做敏感性 | 新增 §2.2.2：3 比值 × 5 档扰动（±30%）+ 通过标准 | `Q3Q4_Method_Architecture.md` §2.2.2 |
| V2-4 | Q4 "2025 年的投入"口径未显式 | 新增设计原则 6：取同期 23,488.02 元 + 4 条理由 | `Q3Q4_Method_Architecture.md` §3.1 原则 6 |
| V2-5 | §5 红线扩充 | 新增红线 13（禁止历史回放用于反事实）+ 红线 14（必须做代理敏感性）| `Q3Q4_Method_Architecture.md` §5 红线 13/14 |
| V2-6 | 决策日志 | DECISION_LOG 追加 D-021/D-022/D-023/D-024 | `DECISION_LOG.md` |

**4 项决策摘要**：
- D-021 FP-Growth 数据源代理：Sheet1 无关键词列 → 用"推广单元-入选项池"代理
- D-022 Q3 校验改写：旧"历史回放"逻辑矛盾 → 改"代理精度"
- D-023 代理敏感性分析：3 比值 × ±30% × 5 档扰动 + 翻转 < 30% + Top-50 overlap ≥ 70%
- D-024 Q4 同期口径：取 23,488.02 元而非全年 142.59 万元

**对原 DoD 的影响**：
- ✅ DoD V1~V12 维持
- 🆕 新增 DoD V2-sens（代理敏感性）：决策变量翻转 < 30% + Top-50 overlap ≥ 70%
...
| 4 | **Q3 投放策略优化** | ✅ **完成（2026-09-12）** + 6 项约束 + 7 项自检（6 PASS + 1 CAVEAT：注册代理 share-Pearson=-0.096 失效，详见 §5.3.4 caveat）| 主交付物 `results/excel/result3.xlsx` 89 行 × 9 列（推广单元×关键词×天，16 天）；MILP 8,640 决策变量 Optimal；双轨关联 100 条；代理精度校验 click/topimp≥0.98（强）/reg=-0.096（失效）；5 张图（5-18 每日投入/5-19 单元×日期热力等）|
| 5 | **Q4 不确定性优化** | ✅ **完成（2026-09-12）** + 7 项自检（7 PASS + 1 精度溢出：投入超 0.06 元 = 0.0003% PuLP 浮点残留，详见 §5.4.5）| 主交付物 `results/excel/result4.xlsx` 39 行 × 9 列（7 天）；Two-Stage SP PoC 20 场景；6 因子 CV 估计（reg CV=1.357 最高）；`q4_6metrics_extended.csv` 显式 6 期望 + 6 CI；同期预算 23,488.02 元严格约束 |
| 终 | **论文数字同步** | ✅ **完成（2026-09-12 17:50）** + 摘要 caveat 修复（注册预测 37,123 标为代理上限）+ Q4 投入精度放宽声明 + 附录 A/B 状态统一 + n_boot 50→100 全修复 | paper.md 1081 行：摘要重写 §5.3.4 注册代理失效诚实声明 + §5.4.5 投入超 0.06 元精度溢出声明 + 附录 A.21a-d/A.22a-b 重命名（与实际 src/ 对齐）|
...
| **D-021** | **FP-Growth 数据源代理** | Sheet1 无关键词列 → 用"推广单元-入选项池（909 词）"代理 | ✅ **2026-09-12 17:27**（BIAS_v2）|
| **D-022** | **Q3 校验改写** | 旧"历史回放"逻辑矛盾 → 改"代理精度"（评估 §2.2.1 代理假设精度）| ✅ **2026-09-12 17:27**（BIAS_v2）|
| **D-023** | **代理敏感性分析** | §2.2.1 三个代理比值 × ±30% 缩放 × 5 档扰动；翻转 < 30% + Top-50 overlap ≥ 70% | ✅ **2026-09-12 17:27**（BIAS_v2）|
| **D-024** | **Q4 同期口径** | 取 2025-09-11~17 同期 23,488.02 元而非全年 142.59 万元 | ✅ **2026-09-12 17:27**（BIAS_v2）|
